...
> **本 SDK 不直接调用 Claude API，而是通过子进程驱动 Claude Code CLI（npm 包 [`@anthropic-ai/claude-code`](https://www.npmjs.com/package/@anthropic-ai/claude-code)）。** 该 CLI 是一个 Node.js 应用，因此你的运行环境必须安装 Node.js 和该 npm 包：

| 依赖 | 版本要求 | 说明 |
|------|---------|------|
| **Node.js** | >= 22 | Claude Code CLI 的运行时 |
| **Claude Code CLI** | 最新版 | npm 包 `@anthropic-ai/claude-code` |
| **Python** | >= 3.9 | 运行本 SDK |

### 部署脚本

在服务器部署时，使用以下脚本自动安装 Claude Code CLI 及其依赖：

```bash
#!/bin/bash
# deploy_claude_code.sh — 部署时自动安装 Claude Code CLI
...
Claude Code CLI 需要 API Key 才能工作，通过环境变量注入（**不要硬编码在代码或镜像中**）：

```bash
# 方式 1：Anthropic API Key
